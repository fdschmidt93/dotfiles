...
function M.buffer_is_terminal(buf)
  return string.find(buf.name, "term://")
end
...
function M.find_buffer_with(substring)
  local windows = api.nvim_tabpage_list_wins(0)
  for _, win in pairs(windows) do
    local buf_nr = api.nvim_win_get_buf(win)
    local buf_name = api.nvim_buf_get_name(buf_nr)
    if buf_name:find(substring) then
      return win, buf_nr, buf_name
    end
  end
end
...
function M.re_start_ipython(side, command)
  ...
  local win, buf_nr, buf_name = M.find_buffer_with "ipython"
  if win ~= nil then
    -- create new buf
